---
id: 4dfb6761-9c27-47a1-9d85-51ea27a54a32
description: >-
  该油猴脚本用于从 LeetCode 网站上抓取题目描述并转换成 Markdown 格式，TNotes.leetcode 中的所有例题的 md
  格式，就是通过这个脚本解析而来的。
---

```mindmap [本节内容]
- 概述
  - 这篇笔记记录了一个爬取 LeetCode 题目描述的油猴脚本
  - 该油猴脚本用于从 LeetCode 网站上抓取题目描述并转换成 Markdown 格式，方便用户复制使用
  - 脚本通过添加在右键菜单中注入一个能一键复制 LeetCode 题目描述为 markdown 文本的菜单项
- TNotes.leetcode 中的题目描述
  - TNotes.leetcode 这个 kb 中的题目描述部分，基本上就是通过以下流程获取的
  - 题目描述的搬运流程
    - 利用这个脚本解析题目描述为 md
    - 手动右键一个个例题解析
    - 手动修复复制下来的一些错误格式
  - 评价
    - 当时 Agent 自动化能力还没有那么高，大量题目描述好像都是中午下班回家后泡着脚听着书无脑地手动迁
```

## 1. 脚本：油猴一键复制 Leetcode 题目描述的实现脚本

::: code-group

````js [scripts/youhou.js]
// ==UserScript==
// @name         力扣题目转Markdown
...
- `scripts/youhou.js` 是将示例部分解析到代码块中（最开始用的是这个）
- `scripts/youhou-2.js` 是将示例部分解析正常解析到 `.md` 中（后来觉得将示例放到代码块中更加合适，于是后续主要用的是这个）

如有其它格式自定义需求，也可基于此脚本自行交给 AI 去修改。
